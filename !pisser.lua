	...
	downloadUrlToFile('http://rubbishman.ru/dev/samp/pisser/version.json', fpath, function(id, status, p1, p2)
		...
		if f then
			...
			if info and info.latest then
				...
				if version > 1 then
					sampAddChatMessage(('[PISSER]: Обнаружено обновление. Что-то исправлено, может что-то сломано. Обновляюсь..'), 0xFFFFF)
					sampAddChatMessage(('[PISSER]: Текущая версия: '..thisScript().version.." Новая версия: "..version), 0xFFFFF)
...
downloadUrlToFile(updatelink, thisScript().path, function(id, status1, p1, p2)
	if status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
	sampAddChatMessage(('Обновление завершено'), 0xFFFFF)
	thisScript():reload()
end
end)
end
