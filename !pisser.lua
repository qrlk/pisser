...
function iwanttopee(peetype, peename, peesurname, peenick)
	...
	function cmdPissAdd(newFriend)
		if param == nil then
			sampShowDialog(987, "Добавить в чёрный список pisser'a", string.format("Ферма №0 \nФерма №1 \nФерма №2 \nФерма №3 \nФерма №4 \nМастерская №0 \nМастерская №1 \nМастерская №2 \nОтменить"), "Выбрать", "Закрыть", 3)
			...
			if f then
				...
				if info and info.latest then
					...
					if version > tonumber(thisScript().version) then
						sampAddChatMessage(('[PISSER]: Обнаружено обновление. AutoReload может конфликтовать. Обновляюсь..'), 0x8B0000)
						sampAddChatMessage(('[PISSER]: Текущая версия: '..thisScript().version..". Новая версия: "..version), 0x8B0000)
						goplay = 2
